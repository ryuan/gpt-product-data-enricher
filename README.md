...
- Batch API currently does not support the web search tool, but this will be built in when it's available. For now, I've tested building a pipeline to synchronously call the Response API on a per-SKU basis, write results, then relay the parsed output as part of the prompt for each payload in the batch. However, the model outputs had overly high rates of hallucinations so I opted to remove the feature.

## Contact

- Happy to float around ideas, answer questions, or collaborate (on this or related projects dealing with leveraging generative AI models to improve tabular/Excel data analyses, modeling, and extraction). Email me at {first_name}.{last_name}@gmail.com - obfuscating a bit to prevent spam, but you get the idea.