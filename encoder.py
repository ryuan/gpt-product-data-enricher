...
class Encoder:
    ...
    def estimate_input_tokens(self, process_order_number: int, system_instructions: str, user_prompt: str, b64_img_uris: List[str], output_schema: Dict) -> int:
        tokens = 0

        tokens += len(self.encoder.encode(system_instructions))
        tokens += len(self.encoder.encode(user_prompt))
        tokens += 70 * len(b64_img_uris)        # 70 tokens limit if image 'detail' is set to 'low' for gpt-5
